...
class Solution {
  public:
    ...
    Node *helper(int pre[],int size,int &i,int l,int h)
    {
        if(i==size || pre[i]<l || pre[i]>h)
        {
            return NULL;
        }
        Node *root=new Node();
        root->data=pre[i++];
        root->left=helper(pre,size,i,l,root->data);
        root->right=helper(pre,size,i,root->data,h);
        return root;
    }
    Node* Bst(int pre[], int size) {
        // code here
        int i=0;
        return helper(pre,size,i,INT_MIN,INT_MAX);
    }
};
